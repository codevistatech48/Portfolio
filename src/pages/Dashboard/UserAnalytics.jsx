...
import { ArrowLeft, FolderOpen, CheckCircle, Clock, GitPullRequest, TrendingUp } from "lucide-react";
...
export default function UserAnalytics() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/api/analytics/user/${user.id}?period=${period}`, {
  ...
  const overviewCards = [
    { label: "Total Projects", value: analytics?.totalProjects || 0, icon: FolderOpen, color: "text-violet-400" },
    { label: "Completed Projects", value: analytics?.completedProjects || 0, icon: CheckCircle, color: "text-green-400" },
    { label: "Projects In Progress", value: analytics?.projectsInProgress || 0, icon: TrendingUp, color: "text-blue-400" },
    { label: "Pending Revisions", value: analytics?.pendingRevisions || 0, icon: Clock, color: "text-amber-400" },
    { label: "Total Revisions Submitted", value: analytics?.totalRevisionsSubmitted || 0, icon: GitPullRequest, color: "text-purple-400" },
  ];
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 pb-12 pt-28 sm:px-8">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-gray-400">Your project statistics and insights</p>
          </div>
        ...
        <div className="mt-8 rounded-3xl border border-white/10 bg-[#151B30]/90 p-6 backdrop-blur-xl">
          ...
          <div className="flex h-64 items-center justify-center text-gray-500">
            <div className="text-center">
              ...
              <p>Detailed project insights coming soon</p>
            </div>
          </div>
        </div>
      </div>
